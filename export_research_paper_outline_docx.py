...
PDF_PATH = PROJECT_ROOT / "major_project_research_paper_outline_format.pdf"
DOCX_PATH = PROJECT_ROOT / "major_project_research_paper_outline_format.docx"
...
def add_paragraph(document: Document, text: str) -> None:
    ...
    if text.startswith("Design and Evaluation of"):
        ...
        run.font.size = Pt(18)
        return
    if text.startswith("Outline-Formatted") or text.startswith("Prepared in the"):
        p = document.add_paragraph()
        p.alignment = WD_ALIGN_PARAGRAPH.CENTER
        run = p.add_run(text)
        run.font.size = Pt(11)
        return
    if len(text) < 90 and text[:1] in "IVX" and ". " in text[:6]:
        p = document.add_paragraph()
        p.style = document.styles["Heading 1"]
        p.add_run(text)
        return
    if len(text) < 100 and text[:2] in ("A.", "B.", "C.", "D."):
        p = document.add_paragraph()
